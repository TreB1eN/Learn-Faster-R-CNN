...
#     pdb.set_trace()
    ...
    dy = loc[:, 0::4] #等价于 loc[:,0].reshape(-1,1)
    dx = loc[:, 1::4] #等价于 loc[:,1].reshape(-1,1)
    dh = loc[:, 2::4] #等价于 loc[:,2].reshape(-1,1)
    dw = loc[:, 3::4] #等价于 loc[:,3].reshape(-1,1)
    # 其实就是把loc里四个值一维取出
    ...
    (16650,)
    (Pdb) src_height[:, xp.newaxis].shape
    (16650, 1)
    """
    ...
    # 接下来再把中心点+宽高 转换成 [y1,x1,y2,x2]的形式
...
def bbox2loc(src_bbox, dst_bbox):
    ...
    * :math:`t_w = \\log(\\frac{g_w} {p_w})`
    ...
    eps = xp.finfo(height.dtype).eps #求一个机器极小值
